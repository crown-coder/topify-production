...
const FormField = ({ label, name, value, onChange, placeholder, error, cardTypeId, type = 'text' }) => (
    <div>
...
const KycModal = ({ provider, onSuccess, onClose, cardTypeId }) => {
    const [KycUserInformation, setKycUserInformation] = useState([]);
    ...
    // Getting the KYC User information
    const fetchKycUserInformation = async () => {
        setIsLoading(true);
        try {
            ...
            // console.log("KYC User Information:", response.data);
            setKycUserInformation(response.data.kyc_info);

            setFormData(prev => ({
                ...prev,
                state: response.data.kyc_info.state || '',
                city: response.data.kyc_info.city || '',
                firstName: response.data.kyc_info.firstname || '',
                lastName: response.data.kyc_info.lastname || '',
                address: response.data.kyc_info.address || '',
                dateOfBirth: response.data.kyc_info.date_of_birth || '',
                postal_code: response.data.kyc_info.postal_code || '',
                house_no: response.data.kyc_info.house_no || '',
                bvn_number: response.data.kyc_info.bvn || '',
                // Set ID type based on available information
                id_type: response.data.kyc_info?.nin ? 'nin' :
                    response.data.kyc_info?.passport_number ? 'passport' : 'bvn'
            }))

        } catch (error) {
            console.error("Error fetching KYC user information:", error);
            setAlert({
                show: true,
                message: "Failed to fetch KYC user information. Please try again later.",
                type: "error"
            });
        } finally {
            setIsLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formDataToSend.append('provider', provider);
            ...
            console.log("The submitted data", formDataToSend)
            ...
            const errorMessage = error.response?.data?.message || error.message || "Failed to complete KYC. Please try again.";
            setAlert({
                show: true,
        ...
        return (
            <div className="w-full p-6 bg-white rounded-xl shadow-sm mt-2 flex justify-center items-center h-64">
                <div className="text-center">
                    ...
                    <p className="mt-3 text-gray-600">Loading your information...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="w-full p-6 bg-white rounded-xl shadow-sm mt-2">
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100">
                    <h3 className="font-medium text-blue-700 mb-3">Why we need KYC?</h3>
                    <ul className="text-sm text-gray-600 space-y-2">
                        {[
                            "To comply with financial regulations",
                            "To protect your account from fraud",
                            "To enable all card features"
                        ].map((item, index) => (
