  ...
  return (
    <div className='p-5 bg-white dark:bg-gray-800 w-full my-2 rounded-xl'>
      ...
      <div className='w-full grid gap-3 grid-cols-1 lg:grid-cols-3 mt-3'>
        <Card className="first:bg-[#4CACF0] dark:first:bg-[#4CACF0] text-slate-100" btn="text-slate-100" icon={<IoTrophyOutline />} title="Main Balance" amount={`${balance}`} button="Smart Earner" content={<button className='text-2xl cursor-pointer' onClick={() => setIsShowBalance(!isShowBalance)}>{isShowBalance ? <BiHide /> : <RxEyeOpen />}</button>} />
        <Card title="Referral Balance" amount="N0.00" button="Refer more people" content={<button className='text-2xl cursor-pointer'><BiHide /></button>} />
